...
/**
 * <p>Title: LY17CICSHandler</p>
 * <p>Description: LY17 CICS Handler - Validate Claim Transaction </p>
...
public class LY17CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C017";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY17Event event = (LY17Event)ce;
    Logger.info("Validating Claim Transaction..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly17");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly17");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY17SessionIDField,Integer.toString(event.getUserSession()));
