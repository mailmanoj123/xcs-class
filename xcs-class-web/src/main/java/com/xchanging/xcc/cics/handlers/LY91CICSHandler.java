...
public class LY91CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C091";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY91Event event = (LY91Event)ce;
    Logger.info(" Build Bulk/CCS Correction Window- LY91 (CO91) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly91");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly91");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY91_SessionID_Field,Integer.toString(event.getUserSession()));
