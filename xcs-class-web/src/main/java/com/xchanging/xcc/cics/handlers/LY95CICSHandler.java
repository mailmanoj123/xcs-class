...
public class LY95CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C095";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY95Event event = (LY95Event)ce;
    Logger.info("Delete Deselected Bulk Components- LY95 (CO95) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly95");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly95");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY95_SessionID_Field,Integer.toString(event.getUserSession()));
