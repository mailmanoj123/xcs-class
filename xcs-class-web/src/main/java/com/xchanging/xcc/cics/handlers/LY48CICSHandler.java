...
import com.xchanging.xcc.events.LY48Event;
...
public class LY48CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C048";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY48Event event = (LY48Event)ce;
    Logger.info("Building R/I Schedule Screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly48");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly48");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY48_SESSION_NO,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY48_PREV_PRESSED,event.getPREV_PRESSED());
        queryRecord.put(Keys.LY48_NEXT_PRESSED,event.getNEXT_PRESSED());
        ...
        MappedRecord aRecord = rf.createMappedRecord(Keys.LY48_START_KEY);

        aRecord.put(Keys.LY48_START_SYS_REF,event.getSTART_SYS_REF());
        aRecord.put(Keys.LY48_START_CURR_NO,event.getSTART_CURR_NO());
        aRecord.put(Keys.LY48_START_SDN_NO,event.getSTART_SDN_NO());
        aRecord.put(Keys.LY48_START_BDOWN_NO,event.getSTART_BDOWN_NO());

        queryRecord.put(Keys.LY48_START_KEY,aRecord);
