...
public class LY40CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C040";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY40Event event = (LY40Event)ce;
    Logger.info("Build Single Claim Loss Reserve Screen- LY40 (CO40) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly40");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly40");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY40SessionIDField,Integer.toString(event.getUserSession()));
