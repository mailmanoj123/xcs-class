...
import com.xchanging.xcc.events.LY23Event;
...
public class LY23CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C023";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY23Event event = (LY23Event)ce;
    Logger.info("Contra Correction Initial Validation-  LY23 (C023) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly23");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly23");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY23_SessionID_Field,Integer.toString(event.getUserSession()));
