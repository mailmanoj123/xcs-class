...
public class LZ24CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C122";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("lz24");
        iSpeq.setExecutionTimeout(WAIT_TIME);
        RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("lz24");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LZ24_SESSION_NO,Integer.toString(event.getUserSession()));
