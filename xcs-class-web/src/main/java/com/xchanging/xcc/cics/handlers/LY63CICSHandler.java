...
import com.xchanging.xcc.events.LY63Event;
...
public class LY63CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C063";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY63Event event = (LY63Event)ce;
    Logger.info("Assign Take Down Numbers- LY63 (CO63) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly63");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly63");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY63_SessionID_Field,Integer.toString(event.getUserSession()));
