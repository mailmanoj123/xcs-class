...
/**
 * <p>Title: LY56CICSHandler</p>
 * <p>Description: LY56 CICS Handler - Build Subsequent Advice Schedule Screen</p>
...
public class LY56CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C056";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY56Event event = (LY56Event)ce;
    Logger.info("Building Subsequent Advice Schedule Screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly56");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly56");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY56SessionIDField,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY56_PREV_PRESSED_FIELD,event.getPREV_PRESSED());
        queryRecord.put(Keys.LY56_NEXT_PRESSED_FIELD,event.getNEXT_PRESSED());
        queryRecord.put(Keys.LY56_START_BDOWN_NO,event.getSTART_BDOWN_NO());
