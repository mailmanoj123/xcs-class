...
/**
 * <p>Title: LY71CICSHandler</p>
 * <p>Description: LY71 CICS Handler - Set up Grouping Session Record</p>
...
public class LY71CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C071";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY71Event event = (LY71Event)ce;
    Logger.info("Setting up Grouping Session Record..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly71");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly71");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY71SessionIDField,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY71_GROUP_SCREEN_FIELD,event.getGROUP_SCREEN());
