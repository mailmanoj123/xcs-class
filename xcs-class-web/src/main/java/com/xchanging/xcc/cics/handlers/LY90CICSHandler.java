...
public class LY90CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C090";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY90Event event = (LY90Event)ce;
    Logger.info(" Bulk/CCS Correction Initial Validation- LY90 (CO90) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly90");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly90");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY90_SessionID_Field,Integer.toString(event.getUserSession()));
