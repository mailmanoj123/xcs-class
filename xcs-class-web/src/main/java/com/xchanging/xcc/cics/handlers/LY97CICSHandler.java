...
import com.xchanging.xcc.events.LY97Event;
...
public class LY97CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C097";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY97Event event = (LY97Event)ce;
    Logger.info("Delete Breakdown " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly97");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly97");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY97SessionIDField,Integer.toString(event.getUserSession()));
