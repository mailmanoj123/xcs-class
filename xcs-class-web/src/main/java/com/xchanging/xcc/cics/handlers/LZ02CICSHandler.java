...
public class LZ02CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C101";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LZ02Event event = (LZ02Event)ce;
    Logger.info("Create Skeleton Breakdown for an Initial- LZ02 (C101) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("lz02");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("lz02");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LZ02_SessionID_Field,Integer.toString(event.getUserSession()));
