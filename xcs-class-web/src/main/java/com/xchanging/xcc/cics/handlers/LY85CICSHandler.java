...
public class LY85CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C085";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY85Event event = (LY85Event)ce;
    Logger.info("Obtain existing diary date..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly85");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly85");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY85_SESSION_NO,Integer.toString(event.getUserSession()));
