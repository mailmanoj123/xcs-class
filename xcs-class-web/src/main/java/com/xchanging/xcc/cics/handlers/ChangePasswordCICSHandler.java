...
public class ChangePasswordCICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C003";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    ChangePasswordEvent event = (ChangePasswordEvent)ce;
    Logger.info("Changing password for session id: " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly03");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly03");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY03SessionIDField,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY03NewPasswordField,event.getNewPassword());
