...
import com.xchanging.xcc.events.LY83Event;
...
public class LY83CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C083";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY83Event event = (LY83Event)ce;
    Logger.info("Building 'CLASS for Lloyds' Response screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly83");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly83");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY83_SESSION_NO,Integer.toString(event.getUserSession()));
