...
public class LZ18CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C117";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    Logger.info("Create Search Screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("lz18");
        iSpeq.setExecutionTimeout(WAIT_TIME);
        RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("lz18");
        ...
        queryRecord.put(Keys.C117_SESSION_NO,SecondSession);
        // The line below has been commented out- as it puts the primary session into the session_no!
        //queryRecord.put(Keys.C117_SESSION_NO,Integer.toString(event.getUserSession()));
