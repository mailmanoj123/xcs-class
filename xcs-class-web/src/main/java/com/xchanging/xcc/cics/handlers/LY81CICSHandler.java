...
import com.xchanging.xcc.events.LY81Event;
...
public class LY81CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C081";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY81Event event = (LY81Event)ce;
    Logger.info("Build CCS Amounts Screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly81");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly81");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY81_SESSION_NO,Integer.toString(event.getUserSession()));
