...
public class LY66CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C066";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY66Event event = (LY66Event)ce;
    Logger.info("Set up Bulk Component Session Record- LY66 (CO66) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly66");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly66");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY66_SessionID_Field,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY66_COMP_SYS_REF_Field,event.getCOMP_SYS_REF());
