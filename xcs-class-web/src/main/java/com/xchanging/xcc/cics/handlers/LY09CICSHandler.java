...
public class LY09CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C009";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY09Event event = (LY09Event)ce;
    Logger.info("Check for Security Notes- LY09 (C009) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly09");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly09");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY09_SessionID_Field,Integer.toString(event.getUserSession()));
