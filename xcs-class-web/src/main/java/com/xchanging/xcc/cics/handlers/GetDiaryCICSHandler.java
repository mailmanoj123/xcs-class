...
public class GetDiaryCICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C087";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    GetDiaryEvent event = (GetDiaryEvent)ce;
    Logger.info("Getting Diary for session : " + event.getUserSession());

      try {

        /**
         * Create a new InteractionSpec and create a RecordFactory for
         * the Diary 'in' parameters
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly87");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly87");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY87SessionIDField,Integer.toString(event.getUserSession()));
