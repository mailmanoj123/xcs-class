...
public class LY96CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C096";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY96Event event = (LY96Event)ce;
    Logger.info("Set File Last Seen Date- LY96 (CO96) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly96");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly96");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY96_SessionID_Field,Integer.toString(event.getUserSession()));
