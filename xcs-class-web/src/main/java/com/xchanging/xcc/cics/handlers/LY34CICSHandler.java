...
public class LY34CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C034";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY34Event event = (LY34Event)ce;
    Logger.info("Build Claim Details-  LY34 (C034) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly34");
        iSpeq.setExecutionTimeout(WAIT_TIME);
        RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly34");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY34_SessionID_Field,Integer.toString(event.getUserSession()));
