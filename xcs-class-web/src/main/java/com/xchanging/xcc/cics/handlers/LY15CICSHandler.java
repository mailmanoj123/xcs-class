...
public class LY15CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C015";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY15Event event = (LY15Event)ce;
    Logger.info("Create Non-Aggregate Group LY15 (C015) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly15");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly15");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY15_SessionID_Field,Integer.toString(event.getUserSession()));
