...
public class LY11CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C011";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY11Event event = (LY11Event)ce;
    Logger.info("Navigation program..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly11");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly11");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY11SessionIDField,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY11PrevProgField,event.getPrevProg());
        queryRecord.put(Keys.LY11SCMAdvCodeField,event.getScmAdvCode());
        queryRecord.put(Keys.LY11ExitIndField,event.getExitInd());
