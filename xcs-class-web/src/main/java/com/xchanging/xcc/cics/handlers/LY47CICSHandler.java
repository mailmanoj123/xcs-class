...
import com.xchanging.xcc.events.LY47Event;
...
public class LY47CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C047";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY47Event event = (LY47Event)ce;
    Logger.info("Create New Breakdown- LY47 (CO47) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly47");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly47");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY47_SessionID_Field,Integer.toString(event.getUserSession()));
