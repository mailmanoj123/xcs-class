...
import com.xchanging.xcc.events.LY80Event;
...
public class LY80CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C080";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY80Event event = (LY80Event)ce;
    Logger.info("Building Euro Conversion Details Screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly80");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly80");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY80_SESSION_NO,Integer.toString(event.getUserSession()));
        /**
         * Execute the query
         */
