...
import com.xchanging.xcc.events.LY54Event;
...
public class LY54CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C054";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY54Event event = (LY54Event)ce;
    Logger.info("Binder Schedule Screen Validation- LY54 (CO54) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly54");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly54");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY54_SessionID_Field,Integer.toString(event.getUserSession()));
