...
/**
 * <p>Title: LY31CICSHandler</p>
 * <p>Description: LY31 CICS Handler - Build Market Details</p>
...
public class LY31CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C031";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY31Event event = (LY31Event)ce;
    Logger.info("Building Market Details..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly31");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly31");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY31SessionIDField,Integer.toString(event.getUserSession()));
