...
public class LogoffCICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C004";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LogoffEvent event = (LogoffEvent)ce;
    Logger.info("Logging off session " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly04");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly04");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY04SessionIDField,Integer.toString(event.getUserSession()));
