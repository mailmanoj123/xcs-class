...
public class LY77CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C077";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY77Event event = (LY77Event)ce;
    Logger.info("Build Rate of Exchange Adjustment Window..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly77");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly77");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY77_SESSION_NO,Integer.toString(event.getUserSession()));
