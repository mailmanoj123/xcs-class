...
/**
 * <p>Title: LY20CICSHandler</p>
 * <p>Description: LY20 CICS Handler - Create Subsequent Advice</p>
...
public class LY20CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C020";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY20Event event = (LY20Event)ce;
    Logger.info("Creating Subsequent Advice..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly20");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly20");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY20SessionIDField,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY20TransTypeField,event.getTransType());
