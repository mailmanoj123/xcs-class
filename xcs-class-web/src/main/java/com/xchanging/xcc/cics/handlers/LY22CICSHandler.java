...
public class LY22CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C022";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY22Event event = (LY22Event)ce;
    Logger.info("Contra Correction Initial Validation-  LY22 (C022) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly22");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly22");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY22_SessionID_Field,Integer.toString(event.getUserSession()));
