...
public class LY86CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C086";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY86Event event = (LY86Event)ce;
    Logger.info("Updating Diary Date..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly86");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly86");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY86_SESSION_NO,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY86_DIARY_DATE,event.getDiaryDate());
