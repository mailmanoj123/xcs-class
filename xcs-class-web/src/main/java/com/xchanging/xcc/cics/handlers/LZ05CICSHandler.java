...
import com.xchanging.xcc.events.LZ05Event;
...
public class LZ05CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C104";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LZ05Event event = (LZ05Event)ce;
    Logger.info("Assign Claims Office References- LZ05 (C104) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("lz05");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("lz05");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LZ05_SessionID_Field,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LZ05_MARKET_CODE_Field, event.getMARKET_CODE()) ;
        queryRecord.put(Keys.LZ05_ORIG_CURR_Field, event.getORIG_CURR()) ;
