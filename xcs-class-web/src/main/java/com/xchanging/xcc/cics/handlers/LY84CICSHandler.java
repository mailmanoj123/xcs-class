...
public class LY84CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C084";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY84Event event = (LY84Event)ce;
    Logger.info("Copy Details from another UCR..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly84");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly84");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY84_SESSION_NO,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY84_UCR,event.getUcr());
        queryRecord.put(Keys.LY84_TR,event.getTr());
