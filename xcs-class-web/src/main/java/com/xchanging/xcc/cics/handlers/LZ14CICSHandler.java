...
/**
 Commarea - C113 (LZ14)
 Program  - Associated Screen/Process:- 'Create Re-advice'
 */

public class LZ14CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C113";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LZ14Event event = (LZ14Event)ce;
    Logger.info("Create Re-advice..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("lz14");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("lz14");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LZ14_SESSION_NO,Integer.toString(event.getUserSession()));

        Vector fieldTable = new Vector(1);

        MappedRecord fieldValues = rf.createMappedRecord("C113_FIELD_VALUES");

        fieldValues.put(Keys.LZ14_CURR_NARR_1,event.getCURR_NARR_1());
        fieldValues.put(Keys.LZ14_NARR_CODE_1,event.getNARR_CODE_1());
        fieldValues.put(Keys.LZ14_CURR_NARR_2A,event.getCURR_NARR_2A());
        fieldValues.put(Keys.LZ14_NARR_CODE_2,event.getNARR_CODE_2());
        fieldValues.put(Keys.LZ14_CURR_NARR_2B,event.getCURR_NARR_2B());
