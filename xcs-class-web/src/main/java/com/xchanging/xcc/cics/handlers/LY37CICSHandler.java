...
/**
 * <p>Title: LY37CICSHandler</p>
 * <p>Description: LY37 CICS Handler - Build Financial Details</p>
...
public class LY37CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C037";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY37Event event = (LY37Event)ce;
    Logger.info("Building Financial Details..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly37");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly37");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY37SessionIDField,Integer.toString(event.getUserSession()));
