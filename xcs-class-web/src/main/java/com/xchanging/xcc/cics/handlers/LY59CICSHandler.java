...
public class LY59CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C059";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY59Event event = (LY59Event)ce;
    Logger.info("Build Non-Settlement Summary Screen- LY59 (CO59) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly59");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly59");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY59_SessionID_Field,Integer.toString(event.getUserSession()));
