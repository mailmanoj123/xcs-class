...
/**
 * <p>Title: LY52CICSHandler</p>
 * <p>Description: LY52 CICS Handler - Check for More Currencies</p>
...
import com.xchanging.xcc.events.LY52Event;
...
public class LY52CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C052";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY52Event event = (LY52Event)ce;
    Logger.info("Checking for More Currencies..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly52");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly52");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY52SessionIDField,Integer.toString(event.getUserSession()));
