...
import com.xchanging.xcc.events.LY49Event;
...
public class LY49CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C049";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY49Event event = (LY49Event)ce;
    Logger.info("R/I Schedule Screen Validation- LY49 (CO49) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly49");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly49");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY49_SessionID_Field,Integer.toString(event.getUserSession()));
