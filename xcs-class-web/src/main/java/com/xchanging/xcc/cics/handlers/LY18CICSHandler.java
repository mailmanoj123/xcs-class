...
public class LY18CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C018";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY18Event event = (LY18Event)ce;
    Logger.info("Delete Claim Transaction-  LY18 (C018) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly18");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly18");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY18_SessionID_Field,Integer.toString(event.getUserSession()));
