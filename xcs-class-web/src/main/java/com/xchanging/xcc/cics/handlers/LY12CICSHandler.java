...
public class LY12CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C012";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY12Event event = (LY12Event)ce;
    Logger.info("Build Navigation Bar...");
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly12");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly12");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY12SessionIDField,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LY12CurrentScreen,event.getCurrentScreen());
