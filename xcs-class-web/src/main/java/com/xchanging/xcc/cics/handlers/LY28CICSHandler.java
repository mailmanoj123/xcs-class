...
public class LY28CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C028";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY28Event event = (LY28Event)ce;
    Logger.info("Build Policy/Risk Details-  LY28 (C028) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly28");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly28");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY28_SessionID_Field,Integer.toString(event.getUserSession()));
