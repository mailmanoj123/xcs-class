...
public class LZ15CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C114";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LZ15Event event = (LZ15Event)ce;
    Logger.info("Create Search Screen..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("lz15");
        iSpeq.setExecutionTimeout(WAIT_TIME);
        RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("lz15");
        ...
        //NOTE
        // We are assuming that we pass in the primary session all the time.
        // Hence if the settlement search window is already open and we come back to this
        // screen then we will NOT put in the secondary mainframe session number, but instead
        // pass in the original primary mainframe session number.
        // In this latter case we would expect the HCSE_session_no returned to be the
        // same as the one we've been using previously.
        queryRecord.put(Keys.C114_SESSION_NO,Integer.toString(event.getUserSession()));
