...
public class LZ09CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C108";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LZ09Event event = (LZ09Event)ce;
    Logger.info("Obtain Additional Bulk Component Details - LZ09 (C108) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("LZ09");
        iSpeq.setExecutionTimeout(WAIT_TIME);
        RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("LZ09");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LZ09_SESSION_NO,Integer.toString(event.getUserSession()));
        queryRecord.put(Keys.LZ09_COMP_SYS_REF,event.getCompSysRef());
