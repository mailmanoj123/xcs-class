...
public class LY67CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C067";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY67Event event = (LY67Event)ce;
    Logger.info("Build Conclusion of Bulk Screen- LY67 (CO67) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly67");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly67");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY67_SessionID_Field,Integer.toString(event.getUserSession()));
