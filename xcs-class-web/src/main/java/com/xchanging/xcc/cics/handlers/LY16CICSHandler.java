...
/**
 * <p>Title: LY16CICSHandler</p>
 * <p>Description: LY16 CICS Handler - Check Transaction Status</p>
...
public class LY16CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C016";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY16Event event = (LY16Event)ce;
    Logger.info("Checking Transaction Status..." + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly16");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly16");

        /**
         * Map the user entered data into the MappedRecord
         */
        queryRecord.put(Keys.LY16SessionIDField,Integer.toString(event.getUserSession()));
        /**
         * Execute the query
         */
