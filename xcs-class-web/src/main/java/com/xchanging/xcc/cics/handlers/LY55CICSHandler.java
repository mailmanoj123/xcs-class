...
import com.xchanging.xcc.events.LY55Event;
...
public class LY55CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C055";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY55Event event = (LY55Event)ce;
    Logger.info("Binder Schedule Screen D/B Update- LY55 (CO55) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly55");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly55");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY55_SessionID_Field,Integer.toString(event.getUserSession()));
