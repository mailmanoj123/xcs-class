...
public class LY98CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C098";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY98Event event = (LY98Event)ce;
    Logger.info("Delete Breakdown- LY98 (CO98) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly98");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly98");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY98_SessionID_Field,Integer.toString(event.getUserSession()));
