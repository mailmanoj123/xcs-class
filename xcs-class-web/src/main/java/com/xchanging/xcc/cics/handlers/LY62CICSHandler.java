...
public class LY62CICSHandler extends CICSHandler implements java.io.Serializable {

  private MappedRecord results;
  private String fieldHdr = "C062";
  ...
  public void perform(ClaimsEvent ce, StateMachine sm) throws GeneralFailureException, ClaimsErrorException, ClaimsWarningException {
    ...
    LY62Event event = (LY62Event)ce;
    Logger.info("Build Settlement Summary Screen- LY62 (CO62) " + event.getUserSession());
      ...
      try {
         ...
         */
        AttuInteractionSpec iSpeq = new AttuInteractionSpec("ly62");
        iSpeq.setExecutionTimeout(WAIT_TIME);
RecordFactory rf = sm.getRecordFactory();
        MappedRecord queryRecord = rf.createMappedRecord("ly62");

        /**
         * Map the user entered data into the MappedRecord
         */

        queryRecord.put(Keys.LY62_SessionID_Field,Integer.toString(event.getUserSession()));
